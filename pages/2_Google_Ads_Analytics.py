...
sample_google_csv = """Campaign name,Ad group name,Cost,Impressions,Clicks,Conversions,Date
Search Campaign,Brand Keywords,180,9500,420,35,2026-04-01
Shopping Campaign,Product Group A,320,16000,610,48,2026-04-02
Remarketing Campaign,Past Visitors,95,4200,180,22,2026-04-03
"""

st.title("Google Ads Analytics")
st.write("Upload a Google Ads CSV export. The app will automatically detect common column names where possible.")

st.download_button(
    label="Download sample Google Ads CSV",
    data=sample_google_csv,
    file_name="sample_google_ads.csv",
    mime="text/csv",
)
...
with col1:
    ...
    ad_group_col = select_column(
        "Ad group name",
        columns,
        ["ad group", "ad group name", "ad_group", "ad_group_name"],
        required=False,
    )
    cost_col = select_column(
        "Spend / cost",
        columns,
        ["cost", "spend", "amount spent", "amount_spent", "total cost"],
    )
...
with col2:
    ...
    conversions_col = select_column(
        "Conversions",
        columns,
        ["conversions", "conv.", "conv", "conversion"],
        required=False,
    )
...
if ad_group_col != "Not available":
    df["Ad Group"] = raw_df[ad_group_col].fillna("Not available").astype(str)
else:
    df["Ad Group"] = "Not available"
...
st.dataframe(
    campaign_table.sort_values("Cost", ascending=False),
